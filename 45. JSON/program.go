...
func main() {
	...
	// decoded data. This map[string]interface{} will hold a map of
	...
	// cast them to their appropriate type. For example here we cast
	// the value in num to the expected float64 type.
	num := dat["num"].(float64)
	fmt.Println(num)
	...
	// We can also decode JSON into custom data types. This has the
	// advantages of adding additional type-safety to our programs
	// and eliminating the need for type assertions when accessing
	// the decoded data.
	...
	// standard out. We can also stream JSON encodings directly
